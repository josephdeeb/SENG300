...
<?php
	...
	if(isset($_POST["submitter"]) and $_POST["submitter"] != ""){
		...
		$reviewer = "";
		$query = "SELECT * FROM journals, users WHERE submitter='$submitter' AND status=4 AND submitter=userName";
		$skip = 0;
	}else if(isset($_POST["reviewer"]) and $_POST["reviewer"] != ""){
		$reviewer = $_POST["reviewer"];
		$submitter = "";
		$query = "SELECT * FROM journals, reviewers, users WHERE reviewer='$reviewer' AND journalName=name AND status=4";
		$skip = 0;
	}else{
		$submitter = "";
		$reviewer = "";
		$query = "SELECT * FROM journals, users WHERE status=4 AND submitter=userName";
//		echo "<p>Please select which journals you wish to view.</p>";
				...
				echo '<tr>
						...
						<td>
							...
							<form action="editorViewComs.php" method="post">
								...
								<input type="hidden" name="reviewer" value='.$reviewer.'>
								<input type="hidden" name="returnPage" value=3>
	...
	echo '
		<div class="core">
		<div class="select1">
			<form action="viewAccepted.php" method="post" enctype="multipart/form-data" required>
